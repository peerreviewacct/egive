...
def run_egive(X, y, model, metric,
  ...
  if feature_imp_njobs == 1:
    print('Calculating partial dependence functions for each feature in order')
  else:
  ...
  ranked = graph_df.groupby(['Feature','Feature_Num'],as_index = False)['Diff'].sum() #apply(lambda x: np.clip(x, a_min = 0, a_max = None).sum())
  ranked['rank'] = ranked['Diff'].rank(ascending = False, method = 'first')
  ...
  pdp_df_combined = pd.DataFrame({'Feature': features, 'Feature_Num': feature_num, \
  ...
  threeway_int_score_matrix = np.zeros((min([int_df_filt.shape[0], twoway_to_threeway_ints]), len(kept_feature_nums)))

  int_tuple_name_list = []
  int_tuple_list = []

  #delayed_funcs = []
  for ix in range(min([int_df_filt.shape[0], twoway_to_threeway_ints])):
  ...
  #Note: higher_order_df just lists the candidate interactions, without any h^2 scores
  ...
  top_df = higher_order_df.copy(deep =True)
  ...
  print('Calculating three-way moderated PDPs')
  ...
  high_int_df_long = high_int_df_long[ (high_int_df_long['Feature Num'] != high_int_df_long['Interaction V1'] ) & (high_int_df_long['Feature Num'] != high_int_df_long['Interaction V2']) & (high_int_df_long['Interaction V1'] != high_int_df_long['Interaction V2'])]
  ...
  high_int_copy = pd.DataFrame(high_int_df_long_3scores)
  high_int_copy.rename({'Feature 2': 'Feature 3', 'Feature 3': 'Feature 2'}, axis = 1, inplace = True)
  high_int_copy = high_int_copy[high_int_df_long_3scores.columns]
  high_int_df_long_3scores = pd.concat([high_int_df_long_3scores, high_int_copy], axis = 0).drop_duplicates()
  ...
  if threeway_int_viz_limit is not None:
    if all_threeway_combinations == False:
      top_threeway_int = high_int_df_long_3scores.sort_values('Threeway Score', ascending = False).iloc[0:threeway_int_viz_limit][['Feature','Feature 2','Feature 3']]
    if all_threeway_combinations == True:
      threeway_int_viz_limit_true = int(np.floor(threeway_int_viz_limit / 6)) #number of possible permutations of each three-way int
      high_int_df_long_3scores['Ordered Key'] = high_int_df_long_3scores[['Feature', 'Feature 2', 'Feature 3']].apply(lambda x: "-".join(sorted(x.astype(str))), axis=1)
      top_ints = high_int_df_long_3scores.sort_values('Threeway Score', ascending = False)['Ordered Key'].unique()[0:threeway_int_viz_limit_true] #get first X unique values

      high_int_df_long_3scores_sorted = high_int_df_long_3scores.sort_values('Threeway Score', ascending = False)
      top_threeway_int = high_int_df_long_3scores_sorted[high_int_df_long_3scores_sorted['Ordered Key'].isin(top_ints)][['Feature','Feature 2','Feature 3','Ordered Key']]

    high_int_df_short = pd.merge(high_int_df_long, top_threeway_int, on = ['Feature','Feature 2','Feature 3'])

    high_int_df_short_copy = pd.DataFrame(high_int_df_short)
    high_int_df_short_copy.rename({'V2 Level': 'V3 Level', 'V3 Level': 'V2 Level', 'Interaction V1': 'Interaction V2', 'Interaction V2': 'Interaction V1',
              'Feature 2': 'Feature 3', 'Feature 3': 'Feature 2'}, axis = 1, inplace = True)
    high_int_df_short_copy = high_int_df_short_copy[high_int_df_short.columns]
    high_int_df_short = pd.concat([high_int_df_short, high_int_df_short_copy], axis = 0).drop_duplicates()

  else:
    high_int_df_short = pd.DataFrame(high_int_df_long)

  ymin = high_int_df_short['Y'].min()
  ymax = high_int_df_short['Y'].max()

  #Final plot
  high_int_pdp_chart = alt.Chart(high_int_df_short, title = 'Plot 6: Three-Way PDPs').mark_line().encode(
  x = alt.X('X'),
